...
#define ADD_FIX(d) do { static_assert(sizeof(d) == sizeof(fix), "Size of fix is not right!"); Assert((static_cast<size_t>(packet_size) + sizeof(d)) < MAX_PACKET_SIZE); int swap = INTEL_INT(d.get_raw()); memcpy(data+packet_size, &swap, sizeof(d) ); packet_size += sizeof(d); } while (false)
...
#define GET_FIX(d) do { std::int32_t swap; memcpy(&swap, data+offset, sizeof(d) ); d = fix::set_raw(INTEL_INT(swap)); offset += sizeof(d); } while(false)
